# 1 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c"
# 29 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c"
# 1 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h" 1
# 98 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h"
...
# 99 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h" 2
# 108 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h"
typedef struct
{
...
# 218 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h"
...
# 265 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.h"
...
# 30 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c" 2
# 1 "./0_Src/4_McHal/Tricore/_Reg/IfxIom_bf.h" 1
# 31 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c" 2
# 1 "./0_Src/1_SrvSw/_Utilities/Ifx_Assert.h" 1
# 32 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c" 2
# 49 "0_Src/4_McHal/Tricore/Iom/Driver/IfxIom_Driver.c"
...
void IfxIom_Driver_disableLamEvent(IfxIom_Driver_Lam *driver)
{
    ...
    {
        module->ECMSELR.U &= ~(1 << (driver->channel + (0)));
    }
    else if (driver->systemEventTriggerThreshold >= 2)
    {
        module->ECMSELR.U &= ~(1 << (driver->accumulatedCounterIndex + (16)));
    }
    else
    {
...
boolean IfxIom_Driver_initLam(IfxIom_Driver_Lam *driver, IfxIom_Driver_LamConfig *config)
{
    ...
    else
    {
...

        {
            ...
            if (driver->systemEventTriggerThreshold == 1)
            {
                module->ECMSELR.U |= 1 << (config->channel + (0));
            }
            else if (driver->systemEventTriggerThreshold >= 2)
            {
                ...
                for (index = 0; index < 4; index++)
                {
                    if ((accumulatedEventUsedMask & (1 << index)) == 0)
                    {
                        ...
                        module->ECMSELR.U |= 1 << (index + (16));

                        module->ECMCCFG.U |= (
                            (driver->channel << (0))
                            | (driver->systemEventTriggerThreshold << (4))
                            ) << (index * (8));
                        break;
                    }
                }
...
void IfxIom_Driver_isLamMonGlitch(IfxIom_Driver_Lam *driver, boolean *risingEdgeGlitch, boolean *fallingEdgeGlitch)
{
    ...
    if (driver->monInput == IfxIom_MonInputSignal_p)
    {
        ...
        *risingEdgeGlitch = (module->FPCESR.U >> (index + (16))) != 0;
        *fallingEdgeGlitch = (module->FPCESR.U >> (index + (0))) != 0;
    }
    else
    {
        *risingEdgeGlitch = 0;
