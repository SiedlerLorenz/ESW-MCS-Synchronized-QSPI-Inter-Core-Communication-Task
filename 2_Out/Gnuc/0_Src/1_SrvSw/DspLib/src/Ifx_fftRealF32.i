# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealF32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealF32.c"
# 32 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealF32.c"
...
# 33 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealF32.c" 2
...
Ifx_fftRealF32_ref (struct Ifx_fftRealF32State * state)
{
    ...
    for (i=1; i<n4; i++) {
        do { assert (0<=(i) && (i)<(n)); switch ((n)) { case 1: (twiddle).real = 1.0f; (twiddle).imag = 0.0f; break; case 2: (twiddle) = Ifx_cruF32[(i)<<9]; break; case 4: (twiddle) = Ifx_cruF32[(i)<<8]; break; case 8: (twiddle) = Ifx_cruF32[(i)<<7]; break; case 16: (twiddle) = Ifx_cruF32[(i)<<6]; break; case 32: (twiddle) = Ifx_cruF32[(i)<<5]; break; case 64: (twiddle) = Ifx_cruF32[(i)<<4]; break; case 128: (twiddle) = Ifx_cruF32[(i)<<3]; break; case 256: (twiddle) = Ifx_cruF32[(i)<<2]; break; case 512: (twiddle) = Ifx_cruF32[(i)<<1]; break; case 1024: (twiddle) = Ifx_cruF32[(i)]; break; default: { float32 phi = -(2.0f*3.1415926535f)*(float32)(i)/((float32)(n)); Ifx_error (IFX_ERR_WARN, "IFX_CRU_F32(%d,%d) not accelerated\n", (i), (n)); (twiddle).real = Ifx_cosF32(phi,IFX_MODE_ALGORITHMIC); (twiddle).imag = Ifx_sinF32(phi,IFX_MODE_ALGORITHMIC); } } } while(0);
...
Ifx_fftRealF32 (struct Ifx_fftRealF32State * state)
{
    enum Ifx_mode mode = state->mode;
    switch (mode) {
# 123 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealF32.c"
