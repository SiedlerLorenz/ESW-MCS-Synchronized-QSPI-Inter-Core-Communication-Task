# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxLudRealF32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxLudRealF32.c"
# 36 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxLudRealF32.c"
...
# 37 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxLudRealF32.c" 2
    ...
    for (j=0; j<n; j++) {
        ...
        for (i=j; i<n; i++) {
            ...
            for (k=0; k<j; k++)
                beta -= mtx[i*n+k]*mtx[k*n+j];
            mtx[i*n+j] = beta;
            alpha = row_scale[i]*((copysignf(((beta)),(1.0f))));
            if (alpha>=alpha_max) {
        ...
        if (mtx[j*n+j] == 0.0) {
            Ifx_error (IFX_ERR_ERROR, "SVD matrix is singular\n");
            mtx[j*n+j] = 1.1920928955078125e-7F;
        }
