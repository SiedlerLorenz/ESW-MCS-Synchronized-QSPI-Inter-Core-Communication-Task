# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealQ15.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealQ15.c"
# 38 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealQ15.c"
...
# 39 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealQ15.c" 2




static void
Ifx_fftRealQ15_ref (struct Ifx_fftRealQ15State * state)
{
...
# 87 "0_Src/1_SrvSw/DspLib/src/Ifx_fftRealQ15.c"
    for (i=1; i<n4; i++) {


     do { float32 phi = -(2.0f*3.1415926535f)*(float32)(i)/((float32)(n)); (twiddle).real = (sint16) (Ifx_cosF32(phi,IFX_MODE_ALGORITHMIC)*32767.0f); (twiddle).imag = (sint16) (Ifx_sinF32(phi,IFX_MODE_ALGORITHMIC)*32767.0f); } while(0);



     do { sint32 a; a = (sint32) (data[i].real) + (sint32) (data[n2-i].real); (h1.real) = (sint16)(a>>1); } while(0);
     do { sint32 a; a = (sint32) (c1) * (sint32) (h1.real); (h1.real) = (a+(1<<14))>>15; } while(0);


     do { sint32 a; a = (sint32) (data[i].imag) - (sint32) (data[n2-i].imag); (h1.imag) = (sint16)(a>>1); } while(0);
     do { sint32 a; a = (sint32) (c1) * (sint32) (h1.imag); (h1.imag) = (a+(1<<14))>>15; } while(0);



     do { sint32 a; a = (sint32) (data[i].imag) + (sint32) (data[n2-i].imag); (h2.real) = (sint16)(a>>1); } while(0);
     do { sint32 a; a = (sint32) (c1) * (sint32) (h2.real); (h2.real) = (a+(1<<14))>>15; } while(0);


     do { sint32 a; a = (sint32) (data[i].real) - (sint32) (data[n2-i].real); (h2.imag) = (sint16)(a>>1); } while(0);
     do { sint32 a; a = (sint32) (c2) * (sint32) (h2.imag); (h2.imag) = (a+(1<<14))>>15; } while(0);





     do { sint32 a; a = (sint32) (twiddle.real) * (sint32) (h2.real); (temp1) = (a+(1<<14))>>15; } while(0);
     do { sint32 a; a = (sint32) (twiddle.imag) * (sint32) (h2.imag); (temp2) = (a+(1<<14))>>15; } while(0);
  data[i].real = h1.real+temp1-temp2;


      do { sint32 a; a = (sint32) (twiddle.real) * (sint32) (h2.imag); (temp3) = (a+(1<<14))>>15; } while(0);
      do { sint32 a; a = (sint32) (twiddle.imag) * (sint32) (h2.real); (temp4) = (a+(1<<14))>>15; } while(0);
...
void
Ifx_fftRealQ15 (struct Ifx_fftRealQ15State * state)
{
    enum Ifx_mode mode = state->mode;
    switch (mode) {
