# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdRealF32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdRealF32.c"
# 39 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdRealF32.c"
...
# 40 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdRealF32.c" 2
# 51 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdRealF32.c"
static float32
pythag(float32 a, float32 b, enum Ifx_mode mode)
{
    float32 absa,absb;
    absa=((copysignf(((a)),(1.0f))));
    absb=((copysignf(((b)),(1.0f))));
    if (absa > absb) return absa*Ifx_sqrtF32(1.0f+((float32)(absb/absa)*(float32)(absb/absa)),mode);
    else return (absb == 0.0f ? 0.0f : absb*Ifx_sqrtF32(1.0f+((float32)(absa/absb)*(float32)(absa/absb)),mode));
}
...
Ifx_mtxSvdRealF32 (struct Ifx_mtxSvdRealF32State * state)
{
    ...
    for (i=0; i<n; i++) {
        ...
        if (i < m) {
            ...
            if (scale!=0.0f) {
                ...
                f=a[i*n+i];
                g = -(copysignf((Ifx_sqrtF32(s,mode)),(f)));
                h=f*g-s;
                a[i*n+i]=f-g;
                for (j=l;j<n;j++) {
        ...
        g=s=scale=0.0f;
        if (i <= m && i != n) {
            for (k=l;k<n;k++) scale += ((copysignf(((a[i*n+k])),(1.0f))));
            if (scale) {
                for (k=l;k<n;k++) {
                    a[i*n+k] /= scale;
                    s += a[i*n+k]*a[i*n+k];
        ...
        anorm=((anorm)>=((((copysignf(((w[i])),(1.0f))))+((copysignf(((rv1[i])),(1.0f))))))?(anorm):((((copysignf(((w[i])),(1.0f))))+((copysignf(((rv1[i])),(1.0f)))))));
    }
    for (i=n-1;i>=0;i--) {
        if (i < n) {
            if (g) {
    ...
    for (k=n-1;k>=0;k--) {
        for (its=1;its<=30;its++) {
            ...
            for (l=k;l>=0;l--) {
                nm=l-1;
                if ((float32)(((copysignf(((rv1[l])),(1.0f))))+anorm) == anorm) {
                    flag=0;
                    break;
                }
                if ((float32)(((copysignf(((w[nm])),(1.0f))))+anorm) == anorm) break;
            }
            if (flag) {
                c=0.0f;
                s=1.0f;
                for (i=l;i<=k;i++) {
                    f=s*rv1[i];
                    rv1[i]=c*rv1[i];
                    if ((float32)(((copysignf(((f)),(1.0f))))+anorm) == anorm) break;
                    g=w[i];
                    h=pythag(f,g,mode);
                    w[i]=h;
                    h=1.0f/h;
