# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
# 31 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
...
# 32 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c" 2
# 41 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
static void
evalQ15(csint16 *data, uint32 n)
{
    ...
    for (kmax=1; kmax<n; kmax=istep) {
        ...
        for (k=0; k<kmax; k++) {
            do { float32 phi = -(2.0f*3.1415926535f)*(float32)(k)/((float32)(istep)); (twiddle).real = (sint16) (Ifx_cosF32(phi,IFX_MODE_ALGORITHMIC)*32767.0f); (twiddle).imag = (sint16) (Ifx_sinF32(phi,IFX_MODE_ALGORITHMIC)*32767.0f); } while(0);
            for (i=k; i<n; i+=istep) {
                j=i+kmax;
                do { sint32 help1,help2; help1 = (sint32) (twiddle).real * (sint32) (data[j]).real - (sint32) (twiddle).imag * (sint32) (data[j]).imag; help2 = (sint32) (twiddle).real * (sint32) (data[j]).imag + (sint32) (twiddle).imag * (sint32) (data[j]).real; if (help1>=(0x7fff<<15)) help1 = (0x7fff<<15); if (help2>=(0x7fff<<15)) help2 = (0x7fff<<15); help1 = (help1+(1<<14))>>15; help2 = (help2+(1<<14))>>15; (temp).real = help1; (temp).imag = help2; } while(0);
                do { sint32 help1,help2; help1 = (sint32)(data[i]).real - (sint32)(temp).real; help2 = (sint32)(data[i]).imag - (sint32)(temp).imag; (data[j]).real = (sint16)(help1>>1); (data[j]).imag = (sint16)(help2>>1); } while(0);
                do { sint32 help1,help2; help1 = (sint32)(data[i]).real + (sint32)(temp).real; help2 = (sint32)(data[i]).imag + (sint32)(temp).imag; (data[i]).real = (sint16)(help1>>1); (data[i]).imag = (sint16)(help2>>1); } while(0);
            }
        }
    }
}
...
Ifx_fftComplexQ15 (struct Ifx_fftComplexQ15State * state)
{
 enum Ifx_mode mode = state->mode;
    switch (mode) {
# 128 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
    case IFX_MODE_OPTIMIZED_ASM :
 {
  ...
  case 16:
    Ifx_fftComplexQ15_16hp (state->y,
          state->x);
    break;

  case 32:
    Ifx_fftComplexQ15_32hp (state->y,
          state->x);
    break;

  case 64:
    Ifx_fftComplexQ15_64hp (state->y,
          state->x);
    break;

  case 128:
    Ifx_fftComplexQ15_128hp (state->y,
          state->x);
    break;

  case 256:
    Ifx_fftComplexQ15_256hp (state->y,
          state->x);
    break;

  case 512:
    Ifx_fftComplexQ15_512hp (state->y,
          state->x);
    break;

  case 1024:
    Ifx_fftComplexQ15_1024hp (state->y,
          state->x);
    break;
  }
 }
 break;
# 179 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ15.c"
