# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_atan2F32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_atan2F32.c"
# 30 "0_Src/1_SrvSw/DspLib/src/Ifx_atan2F32.c"
...
# 31 "0_Src/1_SrvSw/DspLib/src/Ifx_atan2F32.c" 2

static float32
Ifx_atan2F32_ref (float32 y,float32 x)
{
    enum Ifx_mode mode = IFX_MODE_REFERENCE_IMPLEMENTATION;
    if (x > 1.1920928955078125e-7F)
        return Ifx_atanF32 (y/x, mode);
    else if (x < -1.1920928955078125e-7F) {
        if (y >= 0.0f)
            return Ifx_atanF32 (y/x, mode) + 3.1415926535f;
        else
            return Ifx_atanF32 (y/x, mode) - 3.1415926535f;
    } else {
        if (y > 1.1920928955078125e-7F)
            return (3.1415926535f*0.5f);
        else if (y < -1.1920928955078125e-7F)
            return -(3.1415926535f*0.5f);
        else
            return 0.0f;
    }
}
# 64 "0_Src/1_SrvSw/DspLib/src/Ifx_atan2F32.c"
static float32
Ifx_atanF32_optimizedC (float32 y,float32 x, float32 offset)
{
    ...
    if (y2<=x2) {
        ...
        if ((xu.i^yu.i)&0x80000000) {
            ...
            result = -(3.1415926535f*0.5f) - x*y/(y2+0.28f*x2);
        } else {
...
Ifx_atan2F32_optimizedC (float32 y,float32 x)
{
    ...
    if (x > 1.1920928955078125e-7F)
        ...
        if (y >= 0.0f)
            result = Ifx_atanF32_optimizedC (y,x, 3.1415926535f);
        else
            result = Ifx_atanF32_optimizedC (y,x,-3.1415926535f);
    } else {
        if (y > 1.1920928955078125e-7F)
            result = (3.1415926535f*0.5f);
        else if (y < -1.1920928955078125e-7F)
            result = -(3.1415926535f*0.5f);
        else
            result = 0.0f;
