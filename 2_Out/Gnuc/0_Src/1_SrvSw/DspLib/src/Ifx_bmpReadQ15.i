# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_bmpReadQ15.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_bmpReadQ15.c"
# 9 "0_Src/1_SrvSw/DspLib/src/Ifx_bmpReadQ15.c"
...
# 10 "0_Src/1_SrvSw/DspLib/src/Ifx_bmpReadQ15.c" 2
# 22 "0_Src/1_SrvSw/DspLib/src/Ifx_bmpReadQ15.c"
static void
check_uint8 (FILE * f, uint8 ref)
{
    if (ref!=fgetc(f)) {
...
static void
check_uint16 (FILE * f, uint16 ref)
{
    uint16 data;
...
static void
check_uint32 (FILE * f, uint32 ref)
{
    uint32 data;
...
void
Ifx_bmpReadQ15 (FILE * f, sint16 ** data, uint32 * width, uint32 * height)
{
    ...
    *height = read_sint32 (f);
    ...
    assert (*width <= byte_width);
    assert (byte_width < *width+4);
    assert ((byte_width & 0x3) == 0);
    assert (byte_width**height<=image_size);


    *data = (sint16*) malloc (sizeof(sint16)**width**height);
    fseek (f, 0, data_offset);
    for (i=0; i<*height; i++) {
        for (j=0; j<*width; j++)
    ...
    assert ((uint32)ftell (f) <= total_size);
}
