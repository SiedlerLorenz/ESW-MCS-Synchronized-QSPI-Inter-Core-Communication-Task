# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ31.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ31.c"
# 31 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ31.c"
...
# 32 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexQ31.c" 2
...
evalQ31(csint32 *data, uint32 n)
{
    ...
    for (kmax=1; kmax<n; kmax=istep) {
        istep=kmax<<1;
        for (k=0; k<kmax; k++) {
            do { float32 phi = -(2.0f*3.1415926535f)*(float32)(k)/((float32)(istep)); (twiddle).real = (sint32) (Ifx_cosF32(phi,IFX_MODE_ALGORITHMIC)*2147483647.0); (twiddle).imag = (sint32) (Ifx_sinF32(phi,IFX_MODE_ALGORITHMIC)*2147483647.0); } while(0);
            for (i=k; i<n; i+=istep) {
                j=i+kmax;
                do { sint64 help1,help2; help1 = (sint64) (twiddle).real * (sint64) (data[j]).real - (sint64) (twiddle).imag * (sint64) (data[j]).imag; help2 = (sint64) (twiddle).real * (sint64) (data[j]).imag + (sint64) (twiddle).imag * (sint64) (data[j]).real; if (help1>=(0x7fffffffll<<31)) help1 = (0x7fffffffll<<31); if (help2>=(0x7fffffffll<<31)) help2 = (0x7fffffffll<<31); help1 = (help1+(1<<30))>>31; help2 = (help2+(1<<30))>>31; (temp).real = (sint32) help1; (temp).imag = (sint32) help2; } while(0);
                do { sint64 help1,help2; help1 = (sint64)(data[i]).real - (sint64)(temp).real; help2 = (sint64)(data[i]).imag - (sint64)(temp).imag; (data[j]).real = (sint32)(help1>>1); (data[j]).imag = (sint32)(help2>>1); } while(0);
                do { sint64 help1,help2; help1 = (sint64)(data[i]).real + (sint64)(temp).real; help2 = (sint64)(data[i]).imag + (sint64)(temp).imag; (data[i]).real = (sint32)(help1>>1); (data[i]).imag = (sint32)(help2>>1); } while(0);
            }
        }
    }
}
