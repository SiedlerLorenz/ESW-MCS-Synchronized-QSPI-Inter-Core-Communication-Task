# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
# 39 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
...
# 40 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c" 2
# 49 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
void
Ifx_mtxSvdComplexF32 (struct Ifx_mtxSvdComplexF32State * state)
{
    ...
    while (1)
 {
            ...
            z = 0.0F;
            for (i = k; i < m; i++)
                z += (((float32)((a[i*n+k]).real)*(float32)((a[i*n+k]).real))+((float32)((a[i*n+k]).imag)*(float32)((a[i*n+k]).imag)));
            b[k] = 0.0F;
            if (z > tol) {
                z = Ifx_sqrtF32(z,mode);
                b[k] = z;
                loti=k*n+k;
                w = (Ifx_sqrtF32((((float32)((a[loti]).real)*(float32)((a[loti]).real))+((float32)((a[loti]).imag)*(float32)((a[loti]).imag))),IFX_MODE_ALGORITHMIC));
                do { (q).real = (1.0F); (q).imag = (0.0F); } while(0);
                if (w != 0.0F) {
                    do { (q).real = (a[loti]).real / (w); (q).imag = (a[loti]).imag / (w); } while(0);
                }
                lotmp=z+w;
                do { (a[loti]).real = (q).real * (lotmp); (a[loti]).imag = (q).imag * (lotmp); } while(0);
                if (k != np - 1) {
                    for (j = k1; j < np; j++) {
                        do { (q).real = (0.0F); (q).imag = (0.0F); } while(0);
                        for (i = k; i < m; i++) {
                            loti=i*n+k;
                            loti2=i*n+j;
                            do { (qtmp).real = (a[loti]).real * (a[loti2]).real + (a[loti]).imag * (a[loti2]).imag; (qtmp).imag = (a[loti]).real * (a[loti2]).imag - (a[loti]).imag * (a[loti2]).real; } while(0);
                            do { (q).real = (q).real + (qtmp).real; (q).imag = (q).imag + (qtmp).imag; } while(0);
                        }

                        lotmp=z*(z+w);
                        do { (q).real = (q).real / (lotmp); (q).imag = (q).imag / (lotmp); } while(0);
                        for (i = k; i < m; i++) {

                            loti=i*n+k;
                            do { (qtmp).real = (q).real * (a[loti]).real - (q).imag * (a[loti]).imag; (qtmp).imag = (q).real * (a[loti]).imag + (q).imag * (a[loti]).real; } while(0);
                            loti=i*n+j;
                            do { (a[loti]).real = (a[loti]).real - (qtmp).real; (a[loti]).imag = (a[loti]).imag - (qtmp).imag; } while(0);
                        }
                    }
                }
                ...
                loti=k*n+k;
                do { (qtmp).real = (-a[loti].real); (qtmp).imag = (a[loti].imag); } while(0);
                lotmp=(Ifx_sqrtF32((((float32)((a[loti]).real)*(float32)((a[loti]).real))+((float32)((a[loti]).imag)*(float32)((a[loti]).imag))),IFX_MODE_ALGORITHMIC));
                do { (q).real = (qtmp).real / (lotmp); (q).imag = (qtmp).imag / (lotmp); } while(0);
                for (j = k1; j < np; j++) {

                    loti=k*n+j;
                    do { (qtmp).real = (a[loti]).real * (q).real - (a[loti]).imag * (q).imag; (qtmp).imag = (a[loti]).real * (q).imag + (a[loti]).imag * (q).real; } while(0);
                    do { (a[loti]).real = (qtmp.real); (a[loti]).imag = (qtmp.imag); } while(0);
                }
            }
            ...
            z = 0.0F;
            for (j = k1; j < n; j++)
                z += (((float32)((a[k*n+j]).real)*(float32)((a[k*n+j]).real))+((float32)((a[k*n+j]).imag)*(float32)((a[k*n+j]).imag)));
            c[k1] = 0.0F;
            if (z > tol) {
                z = Ifx_sqrtF32(z,mode);
                c[k1] = z;
                w = (Ifx_sqrtF32((((float32)((a[k*n+k1]).real)*(float32)((a[k*n+k1]).real))+((float32)((a[k*n+k1]).imag)*(float32)((a[k*n+k1]).imag))),IFX_MODE_ALGORITHMIC));
                do { (q).real = (1.0F); (q).imag = (0.0F); } while(0);
    ...
    if (nu > 0) {
        for (j = 0; j < nu; j++) {
            for (i = 0; i < m; i++) {
                loti=i*n+j;
                do { (u[loti]).real = (0.0F); (u[loti]).imag = (0.0F); } while(0);
            }
            loti=j*n+j;
            do { (u[loti]).real = (1.0F); (u[loti]).imag = (0.0F); } while(0);
        }
    }
    if (nv > 0) {
        for (j = 0; j < nv; j++) {
            for (i = 0; i < n; i++) {
                loti=i*n+j;
                do { (v[loti]).real = (0.0F); (v[loti]).imag = (0.0F); } while(0);
            }
            loti=j*n+j;
            do { (v[loti]).real = (1.0F); (v[loti]).imag = (0.0F); } while(0);
        }
    }
    ...
    for (k = nM1; k >= 0; k--) {
        ...
        while (1) {
            for (L = k; L >= 0; L--) {
                if (((copysignf(((t[L])),(1.0f)))) <= eps) goto Test;
                if (((copysignf(((s[L - 1])),(1.0f)))) <= eps) break;
            }
            ...
            for (i = L; i <= k; i++) {
                f = sn * t[i];
                t[i] *= cs;
                if (((copysignf(((f)),(1.0f)))) <= eps) goto Test;
                h = s[i];
                ...
                if (nu > 0) {
                    for (j = 0; j < n; j++) {
                        ...
                        x=u[loti].real;
                        y=u[loti2].real;
                        do { (u[loti]).real = (x*cs+y*sn); (u[loti]).imag = (0.0F); } while(0);
                        do { (u[loti2]).real = (y*cs-x*sn); (u[loti2]).imag = (0.0F); } while(0);
                    }
                }
# 296 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
            }
            ...
            for (i = L1; i <= k; i++) {
                ...
                if (nv > 0) {
                    for (j = 0; j < n; j++) {
                        ...
                        x=v[loti].real;
                        w=v[loti2].real;
                        do { (v[loti]).real = (x*cs+w*sn); (v[loti]).imag = (0.0F); } while(0);
                        do { (v[loti2]).real = (w*cs-x*sn); (v[loti2]).imag = (0.0F); } while(0);
                    }
                }
                w = Ifx_sqrtF32(h * h + f * f, mode);
                s[i - 1] = w;
                ...
                if (nu > 0) {
                    for (j = 0; j < n; j++) {
                        ...
                        y=u[loti].real;
                        w=u[loti2].real;
                        do { (u[loti]).real = (y*cs+w*sn); (u[loti]).imag = (0.0F); } while(0);
                        do { (u[loti2]).real = (w*cs-y*sn); (u[loti2]).imag = (0.0F); } while(0);
                    }
                }
# 374 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
            }
            t[L] = 0.0F;
            t[k] = f;
            s[k] = x;
        ...
        for (j = 0; j < n; j++) {
            loti=j*n+k;
            do { (v[loti]).real = (-v[loti].real); (v[loti]).imag = (-v[loti].imag); } while(0);
        }
    }
    ...
    for (k = 0; k < n; k++) {
        ...
        if (nv > 0) {
            for (i = 0; i < n; i++) {
                ...
                loti=i*n+j;
                loti2=i*n+k;
                do { (q).real= (v[loti]).real; (q).imag= (v[loti]).imag; (v[loti]).real= (v[loti2]).real; (v[loti]).imag= (v[loti2]).imag; (v[loti2]).real= (q).real; (v[loti2]).imag= (q).imag; } while(0);
            }
        }
        if (nu > 0) {
            for (i = 0; i < n; i++) {
                ...
                loti=i*n+j;
                loti2=i*n+k;
                do { (q).real= (u[loti]).real; (q).imag= (u[loti]).imag; (u[loti]).real= (u[loti2]).real; (u[loti]).imag= (u[loti2]).imag; (u[loti2]).real= (q).real; (u[loti2]).imag= (q).imag; } while(0);
            }
        }
...
# 441 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
    if (nu > 0) {
        for (k = nM1; k >= 0; k--) {
            if (b[k] == 0.0F) continue;
# 456 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
            loti=k*n+k;
            do { (qtmp).real = (-a[loti].real); (qtmp).imag = (-a[loti].imag); } while(0);
            lotmp=(Ifx_sqrtF32((((float32)((a[loti]).real)*(float32)((a[loti]).real))+((float32)((a[loti]).imag)*(float32)((a[loti]).imag))),IFX_MODE_ALGORITHMIC));
            do { (q).real = (qtmp).real / (lotmp); (q).imag = (qtmp).imag / (lotmp); } while(0);
            for (j = 0; j < nu; j++) {
                loti=k*n+j;
                do { (qtmp).real = (u[loti]).real * (q).real - (u[loti]).imag * (q).imag; (qtmp).imag = (u[loti]).real * (q).imag + (u[loti]).imag * (q).real; } while(0);
                do { (u[loti]).real = (qtmp.real); (u[loti]).imag = (qtmp.imag); } while(0);
            }
            ...
            for (j = 0; j < nu; j++) {
                ...
                for (i = k; i < m; i++) {
                    loti=i*n+k;
                    loti2=i*n+j;
                    do { (qtmp).real = (a[loti]).real * (u[loti2]).real + (a[loti]).imag * (u[loti2]).imag; (qtmp).imag = (a[loti]).real * (u[loti2]).imag - (a[loti]).imag * (u[loti2]).real; } while(0);
                    do { (q).real = (q).real + (qtmp).real; (q).imag = (q).imag + (qtmp).imag; } while(0);
                }
                ...
                for (i = k; i < m; i++) {
                    ...
                    do { (qtmp).real = (q).real * (a[loti]).real - (q).imag * (a[loti]).imag; (qtmp).imag = (q).real * (a[loti]).imag + (q).imag * (a[loti]).real; } while(0);
                    loti=i*n+j;
                    do { (u[loti]).real = (u[loti]).real - (qtmp).real; (u[loti]).imag = (u[loti]).imag - (qtmp).imag; } while(0);
                }
            }
...
# 508 "0_Src/1_SrvSw/DspLib/src/Ifx_mtxSvdComplexF32.c"
            loti=k*n+k1;
            do { (qtmp).real = (-a[loti].real); (qtmp).imag = (a[loti].imag); } while(0);
            lotmp=(Ifx_sqrtF32((((float32)((a[loti]).real)*(float32)((a[loti]).real))+((float32)((a[loti]).imag)*(float32)((a[loti]).imag))),IFX_MODE_ALGORITHMIC));
            do { (q).real = (qtmp).real / (lotmp); (q).imag = (qtmp).imag / (lotmp); } while(0);
            for (j = 0; j < nv; j++) {
                loti=k1*n+j;
                do { (qtmp).real = (v[loti]).real * (q).real - (v[loti]).imag * (q).imag; (qtmp).imag = (v[loti]).real * (q).imag + (v[loti]).imag * (q).real; } while(0);
                do { (v[loti]).real = (qtmp.real); (v[loti]).imag = (qtmp.imag); } while(0);
            }
            for (j = 0; j < nv; j++) {
                do { (q).real = (0.0F); (q).imag = (0.0F); } while(0);
                for (i = k1; i < n; i++) {
                    loti=k*n+i;
                    loti2=i*n+j;
                    do { (qtmp).real = (a[loti]).real * (v[loti2]).real - (a[loti]).imag * (v[loti2]).imag; (qtmp).imag = (a[loti]).real * (v[loti2]).imag + (a[loti]).imag * (v[loti2]).real; } while(0);
                    do { (q).real = (q).real + (qtmp).real; (q).imag = (q).imag + (qtmp).imag; } while(0);
                }
                ...
                for (i = k1; i < n; i++) {
                    ...
                    do { (qtmp).real = (a[loti]).real * (q).real + (a[loti]).imag * (q).imag; (qtmp).imag = (a[loti]).real * (q).imag - (a[loti]).imag * (q).real; } while(0);
                    loti=i*n+j;
                    do { (v[loti]).real = (v[loti]).real - (qtmp).real; (v[loti]).imag = (v[loti]).imag - (qtmp).imag; } while(0);
                }
            }
    ...
    for (i=0;i<m;i++)
        for (j=0;j<n;j++) {
            loti=i*n+j;
            do { (a[loti]).real = (u[loti].real); (a[loti]).imag = (u[loti].imag); } while(0);
        }
