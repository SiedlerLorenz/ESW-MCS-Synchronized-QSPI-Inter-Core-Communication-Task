# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexF32.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexF32.c"
# 31 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexF32.c"
...
# 32 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexF32.c" 2
...
evalF32(cfloat32 * data, uint32 n)
{
    ...
    for (kmax=1; kmax<n; kmax=istep) {
        istep=kmax<<1;
        for (k=0; k<kmax; k++) {
            do { assert (0<=(k) && (k)<(istep)); switch ((istep)) { case 1: (twiddle).real = 1.0f; (twiddle).imag = 0.0f; break; case 2: (twiddle) = Ifx_cruF32[(k)<<9]; break; case 4: (twiddle) = Ifx_cruF32[(k)<<8]; break; case 8: (twiddle) = Ifx_cruF32[(k)<<7]; break; case 16: (twiddle) = Ifx_cruF32[(k)<<6]; break; case 32: (twiddle) = Ifx_cruF32[(k)<<5]; break; case 64: (twiddle) = Ifx_cruF32[(k)<<4]; break; case 128: (twiddle) = Ifx_cruF32[(k)<<3]; break; case 256: (twiddle) = Ifx_cruF32[(k)<<2]; break; case 512: (twiddle) = Ifx_cruF32[(k)<<1]; break; case 1024: (twiddle) = Ifx_cruF32[(k)]; break; default: { float32 phi = -(2.0f*3.1415926535f)*(float32)(k)/((float32)(istep)); Ifx_error (IFX_ERR_WARN, "IFX_CRU_F32(%d,%d) not accelerated\n", (k), (istep)); (twiddle).real = Ifx_cosF32(phi,IFX_MODE_ALGORITHMIC); (twiddle).imag = Ifx_sinF32(phi,IFX_MODE_ALGORITHMIC); } } } while(0);
            for (i=k; i<n; i+=istep) {
                j=i+kmax;
                do { (temp).real = (twiddle).real * (data[j]).real - (twiddle).imag * (data[j]).imag; (temp).imag = (twiddle).real * (data[j]).imag + (twiddle).imag * (data[j]).real; } while(0);
                do { (data[j]).real = (data[i]).real - (temp).real; (data[j]).imag = (data[i]).imag - (temp).imag; } while(0);
                do { (data[i]).real = (data[i]).real + (temp).real; (data[i]).imag = (data[i]).imag + (temp).imag; } while(0);
            }
        }
    }
}
...
Ifx_fftComplexF32 (struct Ifx_fftComplexF32State * state)
{
    ...
    switch (mode) {
# 123 "0_Src/1_SrvSw/DspLib/src/Ifx_fftComplexF32.c"
