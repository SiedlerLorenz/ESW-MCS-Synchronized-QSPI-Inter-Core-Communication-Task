# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_firRealQ15.c"
# 1 "C:\\workspace\\esw_mcs_hightec//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/1_SrvSw/DspLib/src/Ifx_firRealQ15.c"
# 35 "0_Src/1_SrvSw/DspLib/src/Ifx_firRealQ15.c"
...
# 36 "0_Src/1_SrvSw/DspLib/src/Ifx_firRealQ15.c" 2

void Ifx_firRealQ15_ref (struct Ifx_firRealQ15State *);

void
Ifx_firRealQ15_ref (struct Ifx_firRealQ15State * state)
{
    uint32 i, j;
    sint16 * y = state->y;
    sint16 * c = state->c;
    ...
    for (i=0; i<n; i++){
        for (j=0; j<m; j++) {
            int k = i+j;
            sint32 z;
            do { sint32 a; a = (sint32) (c[j]) * (sint32) (x[i]); (z) = (a+(1<<14))>>15; } while(0);
            if (k<n) y[k] += (sint16)z;
            else s[k-n] += (sint16)z;
        }
 }
...
void
Ifx_firRealQ15 (struct Ifx_firRealQ15State * state)
{
