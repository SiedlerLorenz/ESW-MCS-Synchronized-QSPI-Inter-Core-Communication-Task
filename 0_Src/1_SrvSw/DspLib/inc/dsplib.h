 ...
 *
 * Revision: Hélena Fontalba, 01.2016
 */
...
#if IFX_TARGET_TRICORE_APPLICATION_FRAMEWORK
#include <Assert.h>
#define Ifx_assert(P) ASSERT(0,(P))
...
/*! complex number with 16-bit integers (Q15) */
//typedef struct csint16_t  { sint16  real /*!< real part */,imag; /*!< imaginary part */ } csint16;
/*! complex number with 32-bit integers (Q31) */
//typedef struct csint32_t  { sint32  real /*!< real part */,imag; /*!< imaginary part */ } csint32;
/*! complex number with 64-bit integers */
typedef struct csint64_t  { sint64  real /*!< real part */,imag; /*!< imaginary part */ } csint64;
/*! complex number with 32-bit IEEE floats */
//typedef struct cfloat32_t { float32 real /*!< real part */,imag; /*!< imaginary part */ } cfloat32;
...
enum Ifx_mode {
    ...
    IFX_MODE_ALGORITHMIC=IFX_MODE_REFERENCE_IMPLEMENTATION, /*!< algorithmic mode,
                                                             * obsoltete, use IFX_MODE_REFERENCE_IMPLEMENTATION instead. */
    ...
    IFX_MODE_HARDWARE_ACCELERATOR=4, /*!< use hardware accelerator */
...
enum Ifx_error_severity {
    ...
    IFX_ERR_ERROR, /*!< \brief severe failure, continuation is possble */
...
/*!
	The Canny edge detector detects a wide range of edges and reduces noise in the source image sint16 * src. The resulting image is saved in sint16 * dst.
*/
...
void Ifx_imgCanny (struct Ifx_imgCannyState *); /*!< image Canny filter */

/* second batch */
/* Ransac */

/*!
	Ransac is an approximation to estimate parameters of a mathematical model from an input signal which contains outliers. Arbitrary data is picked from the     		data set (* x) and fits them (* fittingfn) and returns a list of models (sint16 ** m) that fit the input data. The distances to the expected model are 		       checked (* distfn). When these are sufficiently small (distance threshold sint16 t)), the data is separated to inliers, which fit to the model and outliers. 		Finally the index of the best fitting model is obtained (uint32 * i). The best model ist the one with the least outliers.
*/

struct Ifx_ransacState {
    sint16 * m; 				/*!< OUT inliers */
    uint32 * inliers; 			/*!< OUT vector[n] inliers */
    uint32 d; 					/*!< IN number of dimensions in input data vector */
    uint32 Npts; 				/*!< IN number of input data sets */
    sint16 * x; 				/*!< IN vector[d*nPts] input data sets */
    void (* fittingfn) 			/*!< IN fitting function */
        (sint16 ** m, 			/*!< OUT vector[d*d] return a list of models that fit the input data */
         uint32 d, 				/*!< number of dimensions in input data set */
         sint16 * x 			/*!< IN vector[d] input data set */
            );
    void (* distfn) 			/*!< IN distance function */
        (uint32 * inliers, 		/*!< OUT vector[n] inlier data sets */
         uint32 * n, 			/*!< OUT number of inliers */
         sint16 ** m, 			/*!< IN vector[d*d] list of models that might fit */
         uint32 * i, 			/*!< OUT index of model that fits the data best */
         uint32 d, 				/*!< number of dimensions in input data set */
         sint16 * x, 			/*!< IN vector[d] input data  */
         sint16 t 				/*!< distance threshold between model and data */
            );
    uint8 (* degenfn) 			/*!< IN returns true if data set is degenerate */
        (uint32 d, 				/*!< number of dimensions in input data set */
         sint16 * x 			/*!< IN vector[d] input data set */
            );
    sint16 t; 					/*!< distance threshold between model and data */
    uint32 maxDataTrials; 		/*!< IN maximum number of attempts to fit (Octave uses 100) */
    uint32 maxTrials;  			/*!< IN maximum number of iterations (Octave uses 1000) */
};
void Ifx_ransac (struct Ifx_ransacState *); /*!< Ransac match */
...
void
Ifx_windowApplyQ31a (struct Ifx_windowApplyQ31State * state);  /*!< complex FFT on Q31 fixed-point data */
